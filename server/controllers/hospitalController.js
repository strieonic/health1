...
/* =========================
   🔎 SEARCH PATIENT (BY arogyamId ID)
========================= */
export const searchPatientByArogyamId = async (req, res) => {
  try {
    const { arogyamId } = req.body;

    const patient = await Patient.findOne({ arogyamId }).select(
      "name arogyamId phone bloodGroup emergencyContact",
    );
...
export const getHospitalPatients = async (req, res) => {
  try {
    const data = await HospitalPatient.find({
      ...
      .populate("patientId", "name arogyamId bloodGroup phone emergencyContact")
